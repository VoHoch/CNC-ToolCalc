...
**Version:** v0.0.1-alpha → v0.1.0-alpha
...
│   ├── models/
│   │   ├── tool.py             # Tool, ToolGeometry
│   │   ├── material.py         # Material, MaterialType
│   │   ├── operation.py        # Operation, OperationType
│   │   ├── calculation.py      # CalculationRequest, Response
│   │   └── enums.py            # CoatingType, SurfaceQuality, etc.
│   ├── api/
│   │   ├── routes/
...
│   ├── services/
│   │   ├── material_service.py # Material data service
│   │   └── operation_service.py # Operation data service
│   └── utils/
...
class OperationType(str, Enum):
    ...
    SLOT_FULL = "SLOT_FULL"
    SLOT_TROCHOIDAL = "SLOT_TROCHOIDAL"  # ⭐ NEW
    # ... 7 more operations
...
├── models/
│   ├── tool.py                 (100 lines)
│   ├── material.py             (120 lines)
│   ├── operation.py            (100 lines)
│   ├── calculation.py          (200 lines - partial)
│   └── enums.py                (150 lines)
├── api/routes/
│   ├── health.py               (30 lines)
│   ├── materials.py            (50 lines)
│   └── operations.py           (50 lines)
├── services/
│   ├── material_service.py     (300 lines - data + logic)
│   └── operation_service.py    (250 lines - data + logic)
└── utils/
...
├── test_materials.py           (80 lines)
├── test_operations.py          (80 lines)
...
- [x] NO Database (in-memory data only)
  - Materials: hardcoded in service
  - Operations: hardcoded in service
...
1. **NO Database** - All data in-memory (Python data structures)
2. **ae/ap NOT parametric** - Calculated values, not exported
3. **13 Operations** - Including SLOT_TROCHOIDAL
4. **7 Materials** - Sorted by hardness
...
echo "🧪 Starting Smoke Test: CR-2025-11-11-003"
...
4. Test operations endpoint:
   ...
   # SLOT group must include SLOT_TROCHOIDAL ✓
   ```
5. Test API docs:
...
6. Test CORS:
   - Frontend (localhost:5173) can call backend ✓
